...
var changeModalState = function changeModalState(state) {
  var windowForm = document.querySelectorAll('.balcon_icons_img'),
      ...
      btnNext = document.querySelectorAll('button[data-calc-btn]');
  Object(_checkNumberInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#width');
  Object(_checkNumberInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#height'); //function for binding events to elements and changing props

  function bindActionToElement(event, elements, prop) {
    elements.forEach(function (item, index) {
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              // if type of input is checkbox - processing 2 states
              index === 0 ? state[prop] = 'Hot' : state[prop] = 'Cold';
              ...
              //means it's width and height inputs
  ...
  bindActionToElement('change', windowType, 'view type');
  bindActionToElement('change', windowProfile, 'Checkbox');
};
...
/* harmony import */ var _checkNumberInputs__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./checkNumberInputs */ "./src/js/modules/checkNumberInputs.js");
...
var forms = function forms(state) {
  var forms = document.querySelectorAll('form'),
      ...
      nameInputs = document.querySelectorAll('input[name="user_name"]'); //phone inputs validator

  Object(_checkNumberInputs__WEBPACK_IMPORTED_MODULE_6__["default"])('input[name="user_phone"]'); //name inputs validator

  nameInputs.forEach(function (input) {
    input.addEventListener('input', function () {
      input.value = input.value.replace(/\d/, "");
    });
  }); //response from server statuses

  var message = {
    success: 'Form submitted successfully  ',
    fail: 'An error occurred, try again later',
    processing: 'Loading...'
  }; //send data to server
...
var images = function images() {
  ...
  bigImage.classList.add('image_restrictions');
...
var modal = function modal() {
  function bindModal(triggerSelector, popUpSelector, closeSelector) {
    ...
    popUp.addEventListener('click', function (event) {
      if (event.target === popUp && closeOverlay) {
        windows.forEach(function (window) {
          window.style.display = 'none';
        });
  ...
  function timerPopUp(selector, time) {
  ...
  bindModal('.feedback_block .phone_link', '.popup', '.popup .popup_close');
...
var timer = function timer(id, deadline) {
  var calcTimeLeft = function calcTimeLeft(end) {
  ...
  var getZeroBeforeNumber = function getZeroBeforeNumber(number) {
    if (number <= 9) {
      return '0' + number;
    } else {
      return number;
  ...
  var renderTimer = function renderTimer(selector, end) {
    ...
    function updateTimer() {
      var time = calcTimeLeft(end);
      days.textContent = getZeroBeforeNumber(time.days);
      hours.textContent = getZeroBeforeNumber(time.hours);
      minutes.textContent = getZeroBeforeNumber(time.minutes);
      seconds.textContent = getZeroBeforeNumber(time.seconds);
